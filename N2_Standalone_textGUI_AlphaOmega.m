...
paths.basePath='/Users/Lab Member/Desktop/DBS Slot Machine Pt/pt_371/'; %CHANGE  
paths.pathOut=[paths.basePath 'sort/'];
paths.pathRaw=[paths.basePath 'rawData/'];
...
%CHANGE LABEL 
paths.patientID='pt_371'; % label in plots

filesToProcess = [ 1];  %which channels to detect/sort
...
extractionThreshold=5.003; %manually change it to the same as the alignmnent method


%% global settings
paramsIn=[];

paramsIn.rawFilePrefix='chan';        % some systems use CSC instead of A
paramsIn.processedFilePrefix='test_';

paramsIn.rawFileVersion = 5; % see defineFileFormat.m for options
paramsIn.samplingFreq = 44000; %only used if rawFileVersion==3
...
%params
paramsIn.blockNrRawFig=[ 10 15 20 25 ];   % for which block will a raw figure be made
